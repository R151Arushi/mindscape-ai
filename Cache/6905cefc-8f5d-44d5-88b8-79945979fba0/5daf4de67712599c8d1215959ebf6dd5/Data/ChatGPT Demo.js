...
print(script.leftHandTracker.isTracking);
    ...
    // if(boxReady) {
        ...
        if(script.camArm.worldSpaceToScreenSpace(left_finger1).distance(script.camPlane.worldSpaceToScreenSpace(boxPos)) < camToBox_dist || script.camArm.worldSpaceToScreenSpace(right_finger1).distance(script.camPlane.worldSpaceToScreenSpace(boxPos)) < camToBox_dist) {
            if(left_finger1.distance(left_finger2) < 2 || right_finger1.distance(right_finger2) < 2) {
                ...
                if(boxRot_move1 && (script.camArm.worldSpaceToScreenSpace(left_finger1).y > script.camPlane.worldSpaceToScreenSpace(boxPos).y || script.camArm.worldSpaceToScreenSpace(right_finger1).y > script.camPlane.worldSpaceToScreenSpace(boxPos).y)) {
                    print("ok1")
                    boxRot_move1 = false;
                    boxRot_move2 = true;
                }
                else if (boxRot_move2 && (script.camArm.worldSpaceToScreenSpace(left_finger1).y < script.camPlane.worldSpaceToScreenSpace(boxPos).y || script.camArm.worldSpaceToScreenSpace(right_finger1).y < script.camPlane.worldSpaceToScreenSpace(boxPos).y)) {
                    print("ok2")
                    boxRot_move1 = true;
                    boxRot_move2 = false;
                }
