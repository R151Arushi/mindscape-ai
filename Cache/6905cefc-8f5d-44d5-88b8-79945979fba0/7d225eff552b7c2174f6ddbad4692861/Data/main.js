...
var onTapEvent = script.question_text.onEditingFinished.add(function(tapEventArgs){
    if(!isAsked) {
        ...
        // script.magicBox.getTransform().setWorldRotation(quat.lookAt(new vec3(0, script.magicBox.getTransform().getWorldPosition().y, 0), new vec3(0,1,0)));

        script.magicBox.getTransform().setWorldPosition(script.camPlane.screenSpaceToWorldSpace(new vec2(0.5,.5), 180))
        script.objLookAt.getTransform().setWorldPosition(new vec3(0, script.magicBox.getTransform().getWorldPosition().y, 0))
...
function tapFun() {
    // print(script.camPlane.screenSpaceToWorldSpace(new vec2(0,0), 40));
        ...
        // script.magicBox.getTransform().setWorldRotation(script.camPlane.getTransform().getWorldRotation());
        // script.magicBox.getTransform().setLocalRotation(quat.lookAt(new vec3(0, 0, 0), vec3.up()));
...
// if (isNaN(flat.x))
// {
// // bug in lookAt makes it fail if vectors are aligned...
// if (hit.normal.y > 0.9) flat = quat.angleAxis(Math.PI / 2, vec3.left());
// else if (hit.normal.y < -0.9) flat = quat.angleAxis(-Math.PI / 2, vec3.left());
// }
// script.magicBox.setWorldRotation(flat);
...
// script.magicBox.getTransform().setLocalRotation(script.deviceTracking.getTransform().getLocalRotation().multiply(quat.fromEulerAngles(0,15,0)));



// // Axis to rotate around
// var axis1 = vec3.right();
// var axis2 = vec3.forward();

// // Rotation we will apply to the object's current rotation
// var rotationToApply = quat.angleAxis(0, axis1);
...
// script.obj.getTransform().setWorldRotation(newRotation);
        ...
        // script.magicBox.getTransform().setWorldRotation(quat.lookAt(new vec3(0,0,0), new vec3(0,-1,0)));
        // script.magicBox.getTransform().setLocalRotation(quat.fromEulerAngles(0,script.camPlane.getTransform().getLocalRotation().y,0));
