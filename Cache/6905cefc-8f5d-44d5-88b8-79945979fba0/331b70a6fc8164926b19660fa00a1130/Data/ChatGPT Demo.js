...
function onUpdate() {
    // if(boxReady && boxUpdateCount < 4) {
        ...
        if(boxRot_move1 && (script.camArm.worldSpaceToScreenSpace(left_finger1).distance(script.camPlane.worldSpaceToScreenSpace(boxPos)) < camToBox_dist || script.camArm.worldSpaceToScreenSpace(right_finger1).distance(script.camPlane.worldSpaceToScreenSpace(boxPos)) < camToBox_dist)) {
