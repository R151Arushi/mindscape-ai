...
function onUpdate() {
    // if(boxReady && boxUpdateCount < 4) {
        ...
        print(script.camPlane.getTransform().getWorldPosition().distance(boxPos))

        // print(left_finger1.distance(left_finger2));

        if(script.camArm.worldSpaceToScreenSpace(left_finger1).distance(script.camPlane.worldSpaceToScreenSpace(boxPos)) < 0.2 || script.camArm.worldSpaceToScreenSpace(right_finger1).distance(script.camPlane.worldSpaceToScreenSpace(boxPos)) < 0.15) {
