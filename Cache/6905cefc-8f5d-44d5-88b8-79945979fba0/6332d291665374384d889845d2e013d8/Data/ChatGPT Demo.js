...
function onUpdate() {
    if(boxReady) {
        ...
        if(script.camArm.worldSpaceToScreenSpace(script.armLeft.getTransform().getWorldPosition()).distance(script.camPlane.worldSpaceToScreenSpace(script.boxHandle.getTransform().getWorldPosition())) < 0.3) {

            if(script.camArm.worldSpaceToScreenSpace(script.armRight.getTransform().getWorldPosition()).distance(script.camPlane.worldSpaceToScreenSpace(script.boxHandle.getTransform().getWorldPosition())) < 0.3) {

                boxReady = false;        var delayedEvent11 = script.createEvent("DelayedCallbackEvent");
                delayedEvent11.bind(function(eventData)
                {
                        global.tweenManager.stopTween(script.main, "box_rot");
